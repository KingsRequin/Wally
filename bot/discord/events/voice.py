...
def register(bot: "WallyDiscord") -> None:
    ...
    async def on_voice_state_update(member, before, after) -> None:
        if member.bot or not bot.social:
            return
        if member.guild.id in bot.config.discord.ignored_guilds:
            return
        if before.channel != after.channel:
            ...
            # voice.py already passes member.id to on_voice_join/leave;
            # register_user is called inside those methods.
