...
class VoiceService:
    ...
    def noms_a_entendre(self) -> list[str]:
        """Le salon d'abord, la communauté ensuite (`voice/noms.py`).

        On parle aussi des absents — une viewer du chat, un habitué parti se
        coucher — et un pseudo ne se prononce pas : `kassandreyunikon` n'aidait
    ...
    def _build_stt_pipeline(self, cfg: VoiceConfig, phrases: list[str]) -> None:
        ...
        if provider in ("remote_stream", "remote-stream", "remote"):
            self._streaming = build_streaming_stt(
                cfg, phrases=phrases, extra_terms=self.noms_a_entendre,
