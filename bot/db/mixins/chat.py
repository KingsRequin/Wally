...
class ChatMixin:
    ...
    async def insert_chat_message(self, sender_id, username, avatar_url, content, is_wally, created_at):
        async with self._conn.execute(
            ...
            (sender_id, username, avatar_url, content, int(is_wally), created_at)) as cursor:
            await self._conn.commit()
            return cursor.lastrowid

    async def load_chat_history(self, limit=50):
        async with self._conn.execute("SELECT * FROM chat_messages ORDER BY id DESC LIMIT ?", (limit,)) as cursor:
    ...
    async def load_chat_messages_for_user(self, discord_id: str, limit: int = 100) -> list[dict]:
        ...
        async with self._conn.execute(
            ...
            (f"discord:{discord_id}", limit)) as cursor:
            return [dict(row) for row in await cursor.fetchall()]
    ...
    async def get_refresh_token(self, token_hash):
        async with self._conn.execute(
            ...
            (token_hash, time.time())) as cursor:
    ...
    async def is_chat_user_banned(self, discord_id: str) -> bool:
        async with self._conn.execute(
            "SELECT 1 FROM chat_bans WHERE discord_id = ?", (str(discord_id),)) as cursor:
            return await cursor.fetchone() is not None

    async def list_chat_bans(self) -> list[dict]:
        async with self._conn.execute(
            "SELECT * FROM chat_bans ORDER BY banned_at DESC") as cursor:
            return [dict(row) for row in await cursor.fetchall()]
