"""L'attaque de virus s'achète 50 000 points de chaîne.

Second volet du §9. L'effet visuel existe (`virus_popup`) ; il s'agit ici de le
...
chemin où le spam ne part pas doit rendre les points ET le dire.
"""
from unittest.mock import AsyncMock, MagicMock

import pytest

from bot.twitch.events.virus_popups import CLE_RECOMPENSE, lancer_spam_virus
...
def _bot(*, narrateur=True, live=True, storm_ok=True):
    ...
    if narrateur:
        ...
        narrateur_obj.show_virus_popups.return_value = storm_ok
...
@pytest.mark.asyncio
async def test_le_spam_part_et_les_points_restent_depenses():
    bot = _bot()
    await lancer_spam_virus(bot, acheteur="rina", reward_id="r1", redemption_id="d1")

    bot.discord_bot.overlay_narrator.show_virus_popups.assert_called_once()
...
async def test_un_refus_de_l_overlay_rend_les_points():
    """`show_virus_popups` peut refuser (écran éteint entre-temps). Son retour
    est LU — sans ça, on annoncerait un spam qui n'a pas eu lieu."""
...
async def test_une_panne_pendant_le_lancement_rend_les_points():
    ...
    bot = _bot()
    bot.discord_bot.overlay_narrator.show_virus_popups.side_effect = RuntimeError("bus mort")
