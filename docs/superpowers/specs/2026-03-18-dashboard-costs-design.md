...
| `GET /top-users` | `days=30&limit=10` | `[{user_id, username, total, count}]` trié par total desc |
...
Pastille rouge sur l'onglet "COÛTS" visible depuis tous les onglets quand le seuil dépasse 80%. Implémenté via polling du endpoint `/alert` au changement d'onglet (pas de SSE dédié — les coûts ne changent pas en temps réel).

### Configuration

Nouveau champ `cost_alert_threshold` dans `config.yaml` (défaut: `25.0`). Éditable dans l'onglet Config existant, section "Bot".
